...
from .utils import ImageEvent, TimeConv as TC
...
class Optimise(ABC):
    ...
    @staticmethod
    def prepare_sequence(events: list[ImageEvent]) -> tuple[npt.NDArray[np.uint8],
        ...
        clut_lens = []
        for event in events:
            img = event.img.convert('P').copy()                
            maps.append(np.asarray(img))
    
            a = np.zeros((256, 4))
            pilpal = list(img.palette.colors.keys())
        ...
        cluts = np.stack(clut) # Stack all CLUT in the sequence
        px_sequences = np.asarray(maps) #All cmaps
    ...
    def solve_sequence(cluts, cmaps, clut_len, **kwargs) -> tuple[npt.NDArray[np.uint8],
        ...
        THRESH = kwargs.get('norm_thresh', 20)
        ...
        for n, (clut, cmap) in enumerate(zip(cluts, cmaps)):
            ...
            for k in range(nmat.shape[0]):
                if np.any(cmap == k) == 0:
                    continue
                for j in range(k):
                    if nmat[k, j] < THRESH and np.any(cmap == j):
                        logging.info(f"CLUT {n}: Merging color {clut[j]} to {clut[k]}.")
                        cmap[cmap == j] = k
                        clut_len[n] -= 1
        ...
        #b = np.ascontiguousarray(sequences).view(np.dtype((np.void,
        #        sequences.dtype.itemsize * sequences.shape[0]*sequences.shape[3])))
        ...
        out_map = np.zeros(sequences.shape[0:2])
        ...
        return out_map.astype(np.uint8), \
            np.asarray(list(seq_sorted.values())[:N_SEQUENCES_MAX])
