...
class PDS(PGSegment):
    ...
    def p_vn(self, p_vn: int) -> int:
        self.payload = (__class__.PDSOff.PAL_VERS_N.value, p_vn % 256)
...
class ODS(PGSegment):
    ...
    def o_vn(self, no_vn: int) -> None:
        self.payload = (__class__.ODSOff.OBJ_VN.value, no_vn % 256)
    ...
    def from_scratch(cls, o_id: int, o_vn: int, width: int, height: int, data: bytes,
        ...
        seg = cls(cls.add_header(bytearray([0, 0, o_vn % 256] + [0]*8), cls._NAME, pts, dts))
        ...
        if len(data) <= MAXLEN_FIRST:
            ...
            for k in range(0, len(data[MAXLEN_FIRST:]), MAXLEN_OTHERS):
                iseg = cls(cls.add_header(bytearray([0, 0, o_vn % 256, 0]), cls._NAME, pts, dts))
